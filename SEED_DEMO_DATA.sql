...
VALUES 
((SELECT id FROM public.users WHERE email='rahul.sales@payoutpower.corp'), (SELECT id FROM public.incentive_schemes WHERE name='Alpha Standard'), CURRENT_DATE),
((SELECT id FROM public.users WHERE email='priya.sales@payoutpower.corp'), (SELECT id FROM public.incentive_schemes WHERE name='Beta Tiered'), CURRENT_DATE),
((SELECT id FROM public.users WHERE email='amit.sales@payoutpower.corp'), (SELECT id FROM public.incentive_schemes WHERE name='Gamma Per Unit'), CURRENT_DATE)
...
VALUES 
((SELECT id FROM public.users WHERE email='rahul.sales@payoutpower.corp'), 'Global Corp', 100000, 'Cloud Suite', CURRENT_DATE - INTERVAL '5 days', (SELECT id FROM public.incentive_schemes WHERE name='Alpha Standard'), 5000, 'earned'),
((SELECT id FROM public.users WHERE email='rahul.sales@payoutpower.corp'), 'Nexus Ltd', 250000, 'Security Pack', CURRENT_DATE - INTERVAL '10 days', (SELECT id FROM public.incentive_schemes WHERE name='Alpha Standard'), 12500, 'earned');

-- Priya's tiered sales (One below threshold, one above)
INSERT INTO public.sales_logs (salesperson_id, client_name, deal_value, product, sale_date, scheme_id, calculated_commission, status)
VALUES 
((SELECT id FROM public.users WHERE email='priya.sales@payoutpower.corp'), 'Small Biz Inc', 200000, 'ERP Basic', CURRENT_DATE - INTERVAL '3 days', (SELECT id FROM public.incentive_schemes WHERE name='Beta Tiered'), 10000, 'earned'),
((SELECT id FROM public.users WHERE email='priya.sales@payoutpower.corp'), 'Mega Tech', 800000, 'ERP Enterprise', CURRENT_DATE - INTERVAL '8 days', (SELECT id FROM public.incentive_schemes WHERE name='Beta Tiered'), 64000, 'earned');
...
VALUES 
((SELECT id FROM public.users WHERE email='amit.sales@payoutpower.corp'), 'Hardware Mart', 15000, 'Terminals', CURRENT_DATE - INTERVAL '12 days', (SELECT id FROM public.incentive_schemes WHERE name='Gamma Per Unit'), 10000, 'earned', 20);
